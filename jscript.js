...
class Book {
  ...
  info() {
    let string;
    if (this.read)
      string = `${this.title} by ${this.author}, ${this.pages} pages, read`;
    else
      string = `${this.title} by ${this.author}, ${this.pages} pages, not read yet`;
    return string;
  }
...
class Library {
  ...
  removeBook(id) {
    ...
    bookCard.remove();
    const collection = document.getElementsByClassName("book-card");
